...
if __name__ == "__main__":
    ...
    net = network.CategoricalDQN(env.observation_space.shape, env.action_space.n, ATOMS, VMIN, VMAX).to(device)
    tgt_net = network.CategoricalDQN(env.observation_space.shape, env.action_space.n, ATOMS, VMIN, VMAX).to(device)
    ...
    while True:
        ...
        optimizer.zero_grad()
        loss_t = distribution.calc_loss(net, tgt_net, buffer, VMIN, VMAX, ATOMS, params["gamma"], params["batch_size"], device = device)
        loss_t.backward()
        optimizer.step()
