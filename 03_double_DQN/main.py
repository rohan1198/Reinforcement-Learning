...
if __name__ == "__main__":
    ...
    net = network.DDQN(env.observation_space.shape, env.action_space.n).to(device)
    tgt_net = network.DDQN(env.observation_space.shape, env.action_space.n).to(device)

    writer = SummaryWriter(comment = "-" + params["run_name"])
    print(net)

    buffer = experience.replay_buffer(capacity = params["replay_size"])
    ...
    while True:
        ...
        loss_t = common.calc_loss_double_dqn(batch, net, tgt_net, gamma = params["gamma"], device = device, double = True)
        loss_t.backward()
        optimizer.step()
